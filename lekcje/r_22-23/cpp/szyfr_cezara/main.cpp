...
string odszyfruj(int o_ile, string zaszyfrowana_wiadomosc) {
    ...
    for (int i = 0; i < zaszyfrowana_wiadomosc.length(); i++) {
        if ((zaszyfrowana_wiadomosc[i] >= 'a') &
            (zaszyfrowana_wiadomosc[i] <= 'z')) {
            if (((zaszyfrowana_wiadomosc[i] - o_ile) - 'a') < 0) {
                odszyfrowana_wiadomosc +=
                    zaszyfrowana_wiadomosc[i] - o_ile - 'a' + 26 + 'a';
            } else {
                odszyfrowana_wiadomosc +=
                    zaszyfrowana_wiadomosc[i] - o_ile - 'a' + 'a';
            }
        } else if ((zaszyfrowana_wiadomosc[i] >= 'A') &
                   (zaszyfrowana_wiadomosc[i] <= 'Z')) {
            if (((zaszyfrowana_wiadomosc[i] - o_ile) - 'A') < 0) {
                odszyfrowana_wiadomosc +=
                    zaszyfrowana_wiadomosc[i] - o_ile - 'A' + 26 + 'A';
