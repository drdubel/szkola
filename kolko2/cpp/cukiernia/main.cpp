...
int ciastkowybieracz(int pieniadze, int il_przedmiotow, int masa[], int wartosc[])
{
    int i, wt;
    ...
    for (i = 0; i <= il_przedmiotow; i++)
    {
        for (wt = 0; wt <= pieniadze; wt++)
        {
            if (i == 0 || wt == 0)
                ...
                tablica_wynikow[i][wt] = max(masa[i - 1] + tablica_wynikow[i - 1][wt - wartosc[i - 1]], tablica_wynikow[i - 1][wt]);
            else
